...
window.loadAssignments = async function(courseId, hasAccess) {
    ...
    try {
        ...
        assignments.sort((a, b) => (b.createdAt||'') > (a.createdAt||'') ? 1 : -1);
        ...
        assignments.forEach(function(assign) {
            ...
            const actionBtn = !isSubmitted
                ? '<button onclick="window.showSubmitAssignment(\'' + assign.id + '\', \'' + courseId + '\')" style="background:var(--main); color:white; border:none; padding:8px 16px; border-radius:10px; cursor:pointer; font-weight:bold; font-family:\'Cairo\'; font-size:0.85rem; white-space:nowrap;"><i class="fas fa-paper-plane" style="margin-left:5px;"></i>تسليم الواجب</button>'
                : '<button onclick="window.viewSubmission(\'' + assign.id + '\')" style="background:var(--success); color:white; border:none; padding:8px 16px; border-radius:10px; cursor:pointer; font-weight:bold; font-family:\'Cairo\'; font-size:0.85rem; white-space:nowrap;"><i class="fas fa-eye" style="margin-left:5px;"></i>عرض التسليم</button>';
