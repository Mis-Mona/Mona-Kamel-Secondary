...
// استخدام escapeHTML من script.js بدلاً من تكرارها
function safeText(str) {
    return typeof window.escapeHTML === 'function'
        ? window.escapeHTML(str)
        : (str == null ? '' : String(str).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;').replace(/'/g, '&#039;'));
}
...
function showNotificationToast(notification) {
    ...
    toast.innerHTML = `
        ...
        <div class="notification-toast-content">
            <div class="notification-toast-title">${window.escapeHTML(notification.title)}</div>
            <div class="notification-toast-message">${window.escapeHTML(notification.message)}</div>
                ...
                html += `
                    <div class="notification-item ${notif.read ? '' : 'unread'}" data-id="${notif.id}" onclick="window.markNotificationRead('${notif.id}')">
                        ...
                        <div class="notification-content">
                            <div class="notification-title">${window.escapeHTML(notif.title)}</div>
                            <div class="notification-message">${window.escapeHTML(notif.message)}</div>
                            <div class="notification-time">${window.escapeHTML(notif.createdAtLocal)}</div>
                        </div>
